...
| 담당 | 구분 | 주요 업무 |
|:----:|------|----------|
| 전원 | 공통 업무 | 프로젝트 전체 기획 및 전반 아키텍처 설계, 요구사항 정의, 테스트 기획 및 워크플로우 설계, 문서화 및 산출물 정리 |
| 황도경 | AI 워크플로우 제어 | 고객 요청의 상태에 따라 AI 응답 흐름을 동적으로 제어 |
| 송유선 | 서비스 백엔드 및 지식 검색 | AI 응답 생성을 위한 서버 로직 및 지식 검색 파이프라인 구축 |
| 윤희상 | 음성 인터랙션 (STT/TTS/VAD) | 고객 발화와 시스템 응답을 안정적으로 연결하는 실시간 음성 인터페이스 구성 |
| 김두훈 | 의도 분석 및 분류 | 고객 발화를 의미 단위로 해석하여 서비스 분기를 위한 판단 기준 제공 |
| 김두훈 | 사용자 및 상담원 인터페이스 | 고객과 상담원이 AI 시스템을 직관적으로 활용할 수 있는 화면 구성 |
