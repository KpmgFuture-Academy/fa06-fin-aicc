...
**중요: 검색된 문서의 실제 내용을 꼼꼼히 읽고 분석하여 판단하세요. 유사도 점수는 참고용일 뿐이며, 실제 문서 내용이 고객 질문에 얼마나 관련 있고 답변을 제공할 수 있는지가 핵심입니다.**

문맥 의도는 질문의 주제를 나타냅니다 (예: "대출", "예금", "카드론" 등).
검색된 문서에는 출처, 페이지, 유사도 점수, 그리고 **전체 문서 내용**이 포함되어 있습니다.

다음 기준에 따라 판단하세요:

1. AUTO_HANDLE_OK (자동 처리 가능 - 답변 생성):
   - **검색된 문서 내용을 읽어보니 고객 질문에 대한 명확하고 구체적인 답변이 포함되어 있는 경우**
   - 문서 내용이 고객 질문을 충분히 해결할 수 있는 정보를 제공하는 경우
   - 단순 정보 문의이고 문서에서 답을 찾을 수 있는 경우 (예: "카드론 한도가 얼마야?", "대출 금리는?", "예금 이자는?")
   - FAQ 형태의 질문이고 문서 내용으로 답변 가능한 경우
   - 문서 내용이 명확하고 구체적인 정보를 제공하는 경우

2. NEED_MORE_INFO (추가 정보 필요 - 질문 생성):
   - **검색된 문서 내용을 읽어보니 관련은 있으나 질문에 대한 답변이 불완전하거나 모호한 경우**
   - 문서 내용이 부분적으로만 관련 있거나, 추가 정보가 필요한 경우
   - 질문이 모호하여 문서 내용만으로는 명확한 답변을 제공하기 어려운 경우
   - 고객의 의도가 명확하지 않아 추가 질문이 필요한 경우
   - 문서 내용이 있으나 고객의 구체적인 상황을 파악하기 위해 추가 정보가 필요한 경우

3. HUMAN_REQUIRED (상담사 연결 필요):
   - 고객 메시지에 명확한 불만이나 민원 표현이 있는 경우 (예: "불만", "민원", "화가 났어요")
   - **검색된 문서가 전혀 없거나, 문서 내용을 읽어보니 질문과 전혀 관련 없는 경우**
   - 복잡한 계약 변경이나 대출 신청 같은 경우
   - 개인정보 확인이 필요한 경우
   - **문서 내용을 읽어보니 내용만으로는 처리할 수 없는 복잡한 업무인 경우**
   - 문서 내용이 있으나 개인화된 정보나 실시간 데이터가 필요한 경우

**판단 프로세스:**
1. 먼저 검색된 문서의 전체 내용을 꼼꼼히 읽으세요
2. 문서 내용이 고객 질문에 대한 답변을 제공할 수 있는지 평가하세요
3. 유사도 점수보다는 실제 문서 내용의 관련성과 완전성을 기준으로 판단하세요
4. 문서 내용만으로 답변이 가능하면 AUTO_HANDLE_OK, 부분적으로만 가능하면 NEED_MORE_INFO, 불가능하면 HUMAN_REQUIRED
