...
def get_sample_flow_data():
    ...
    Returns:
        List[Dict]: Flow 테스트 데이터
    """
    from ..metrics.flow_metrics import FlowTestCase, FlowResult
    ...
    for case in data.get("test_cases", []):
        test_case = FlowTestCase(
            ...
            expected_latencies=case.get("expected_latency_ms", {})
        )
        ...
        # 레이턴시 시뮬레이션
        actual_latencies = {}
        for node, expected_ms in case.get("expected_latency_ms", {}).items():
            # ±20% 변동
            variation = random.uniform(0.8, 1.2)
            actual_latencies[node] = int(expected_ms * variation)

        result = FlowResult(
            flow_id=case["flow_id"],
            actual_flow=actual_flow,
            node_latencies=actual_latencies,
            completed=True,
            had_infinite_loop=False
        )
